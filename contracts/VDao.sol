...
interface iVROUTER {
    function ivPool(address) external view returns(bool);
}
interface iVPOOL {
...
contract VDao {
    ...
    function setGenesivRouter(address genesivRouter) public onlyDeployer {
        _router = genesivRouter;
        DEPLOYER = address(0);
    }
    ...
    function lock(address pool, uint256 amount) public nonReentrant {
        require(iVROUTER(_router).ivPool(pool) == true, "Must be listed");
        require(amount > 0, "Must get some");
        if (!isMember[msg.sender]) {
            arrayMembers.push(msg.sender);
            isMember[msg.sender] = true;
        }
        require(iVPOOL(pool).transferTo(address(this), amount),"Must transfer"); // Uni/Bal LP tokens return bool
